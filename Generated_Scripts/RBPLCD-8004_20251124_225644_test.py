...
def test_rbplcd_8004(page: Page):
    ...
    # Selector discovered by: Agent L1 (confidence: 0.87)
    # Action type: click
    ...
    # Selector discovered by: Agent L3 (confidence: 0.90)
    ...
    locator = page.get_by_text("default_Measurement01", exact=False)
    if locator.count() > 1:
        locator = locator.nth(0)
    expect(locator).to_be_visible()
    print("[OK] Verified text: default_Measurement01")

    # Record step result
    step_results.append({
        'step_number': 4,
        'step_text': 'Verify teststep name default_Measurement01 is displayed in details header',
        'verified_text': "default_Measurement01",
        'agent_used': 'L3',
        'confidence': 0.90,
        'status': 'PASSED'
    })
    ...
    # Wait for element to be available
    page.wait_for_selector("[data-detailstitle='detailsTitle']", timeout=10000)

    # Click action
    page.click("[data-detailstitle='detailsTitle']")
    page.wait_for_load_state('networkidle')
    print("[OK] Clicked element")
    ...
    step_results.append({
        ...
        'selector': "[data-detailstitle='detailsTitle']",
        'agent_used': 'L1',
        'confidence': 0.75,
        'status': 'PASSED'
    })
    ...
    # Text verification (L3 Vision)
    # Use nth(0) to handle multiple matches (e.g., text in table + tooltip)
    locator = page.get_by_text("Id", exact=False)
    if locator.count() > 1:
        locator = locator.nth(0)
    expect(locator).to_be_visible()
    print("[OK] Verified text: Id")
