...
def test_rbplcd_8004(page: Page):
    ...
    # Use .first() to handle multiple matches (e.g., text in table + tooltip)
    expect(page.get_by_text("default_Measurement01", exact=False).first()).to_be_visible()
    ...
    # Text verification (L3 Vision)
    # Use .first() to handle multiple matches (e.g., text in table + tooltip)
    expect(page.get_by_text("Id", exact=False).first()).to_be_visible()
